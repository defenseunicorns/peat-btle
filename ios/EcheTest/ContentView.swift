...
//  EcheTest
//
//  Main view for Eche BLE mesh demo
//  Mirrors the Android EcheDemo MainActivity layout
//

import SwiftUI

struct ContentView: View {
    @EnvironmentObject var viewModel: EcheViewModel
...
struct StatusHeaderView: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            // Mesh ID badge
            Text("Mesh: \(EcheViewModel.MESH_ID)")
                .font(.caption)
                .fontWeight(.bold)
                ...
                HStack {
                    ...
                    if let meshId = peer.meshId {
                        Text("Mesh: \(meshId)")
                            ...
                            .background(meshId == EcheViewModel.MESH_ID ? Color.green.opacity(0.2) : Color.orange.opacity(0.2))
                            .cornerRadius(2)
                    }
                }
        ...
        VStack(alignment: .leading, spacing: 6) {
            if !viewModel.ackStatus.ackedNodes.isEmpty {
                HStack {
                    Text("✓ ACK'd:")
                        .foregroundColor(.green)
                    Text(viewModel.ackStatus.ackedNodes.map { String(format: "Eche-%08X", $0) }.joined(separator: ", "))
                        .font(.caption)
                }
            }
            ...
            if !viewModel.ackStatus.waitingNodes.isEmpty {
                HStack {
                    ...
                    Text(viewModel.ackStatus.waitingNodes.map { String(format: "Eche-%08X", $0) }.joined(separator: ", "))
                        .font(.caption)
                }
            }
        }
...
#Preview {
    ContentView()
        .environmentObject(EcheViewModel())
}
