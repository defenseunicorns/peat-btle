...
//! UniFFI bindings for peat-btle on Apple platforms
//!
//! This crate provides Swift bindings for the Peat BLE library using UniFFI.

use std::collections::HashMap;
use std::sync::{Arc, Mutex, RwLock};
use std::time::SystemTime;

use peat_btle::config::{BleConfig, BlePhy as PeatBlePhy, DiscoveryConfig, MeshConfig};
use peat_btle::platform::apple::CoreBluetoothAdapter;
use peat_btle::platform::BleAdapter;
use peat_btle::{NodeId as PeatNodeId, DEFAULT_MESH_ID};
...
pub fn init_logging() {
    let _ = env_logger::builder()
        ...
        .try_init();
    log::info!("peat-apple-ffi initialized");
}
...
#[uniffi::export]
pub fn parse_peat_device_name(name: String) -> Option<ParsedDeviceName> {
...
#[uniffi::export]
pub fn generate_peat_device_name(mesh_id: String, node_id: u32) -> String {
    let config = MeshConfig::new(mesh_id);
    config.device_name(PeatNodeId::new(node_id))
}
...
impl From<PeatBlePhy> for BlePhy {
    fn from(phy: PeatBlePhy) -> Self {
        match phy {
            PeatBlePhy::Le1M => BlePhy::Le1M,
            PeatBlePhy::Le2M => BlePhy::Le2M,
            PeatBlePhy::LeCodedS2 | PeatBlePhy::LeCodedS8 => BlePhy::LeCoded,
        }
    }
}
...
impl From<peat_btle::error::BleError> for PeatError {
    fn from(err: peat_btle::error::BleError) -> Self {
        match err {
            peat_btle::error::BleError::NotSupported(_) => PeatError::BluetoothUnavailable,
            peat_btle::error::BleError::ConnectionFailed(msg) => {
                PeatError::ConnectionFailed { reason: msg }
            }
            peat_btle::error::BleError::Timeout => PeatError::Timeout,
            peat_btle::error::BleError::PlatformError(msg) => PeatError::Internal { message: msg },
            _ => PeatError::Internal {
...
impl PeatAdapter {
    ...
    #[uniffi::constructor]
    pub fn new(node_id: u32) -> Result<Arc<Self>, PeatError> {
        Self::with_mesh_id(node_id, DEFAULT_MESH_ID.to_string())
    }

    /// Create a new adapter with a specific mesh ID
    #[uniffi::constructor]
    pub fn with_mesh_id(node_id: u32, mesh_id: String) -> Result<Arc<Self>, PeatError> {
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .worker_threads(2)
            .enable_all()
            .build()
            .map_err(|e| PeatError::Internal {
                message: format!("Failed to create runtime: {}", e),
            })?;

        log::info!(
            "Creating PeatAdapter with node ID: {:08X}, mesh ID: {}",
            node_id,
            mesh_id
        );

        Ok(Arc::new(PeatAdapter {
            node_id,
            mesh_id,
    ...
    pub fn get_device_name(&self) -> String {
        ...
        config.device_name(PeatNodeId::new(self.node_id))
    }
    ...
    pub fn start(&self) -> Result<(), PeatError> {
        let mut state = self.state.write().unwrap();
        if state.is_running {
            return Err(PeatError::AlreadyInitialized);
        }

        log::info!("Starting PeatAdapter...");

        // Create the CoreBluetooth adapter
        let adapter = CoreBluetoothAdapter::new().map_err(|e| PeatError::Internal {
        ...
        *self.adapter.write().unwrap() = Some(adapter);
        ...
        log::info!("PeatAdapter started successfully");
        Ok(())
    }

    /// Stop the adapter
    pub fn stop(&self) -> Result<(), PeatError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }

        log::info!("Stopping PeatAdapter...");
    ...
    pub fn start_discovery(&self) -> Result<(), PeatError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }
    ...
    /// Stop scanning
    pub fn stop_discovery(&self) -> Result<(), PeatError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }
    ...
    pub fn start_advertising(&self) -> Result<(), PeatError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }
    ...
    /// Stop advertising
    pub fn stop_advertising(&self) -> Result<(), PeatError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }
    ...
    pub fn connect(&self, identifier: String) -> Result<(), PeatError> {
        ...
        let node_id = peer
            ...
            .ok_or_else(|| PeatError::ConnectionFailed {
        ...
        let adapter_guard = self.adapter.read().unwrap();
        if let Some(adapter) = adapter_guard.as_ref() {
            let peat_node_id = PeatNodeId::new(node_id);
            self.runtime
                .block_on(async { adapter.connect(&peat_node_id).await })
                .map_err(|e| PeatError::ConnectionFailed {
                    reason: e.to_string(),
                })?;
        }
    ...
    pub fn disconnect(&self, peer_id: u32) -> Result<(), PeatError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }
        ...
        let adapter_guard = self.adapter.read().unwrap();
        if let Some(adapter) = adapter_guard.as_ref() {
            let node_id = PeatNodeId::new(peer_id);
            self.runtime
    ...
    pub fn send_data(&self, peer_id: u32, data: Vec<u8>) -> Result<(), PeatError> {
        let state = self.state.read().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }
        if !state.connections.contains_key(&peer_id) {
            return Err(PeatError::ConnectionFailed {
    ...
    pub fn broadcast_data(&self, data: Vec<u8>) -> Result<(), PeatError> {
        let state = self.state.read().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }
    ...
    pub fn trigger_sync(&self) -> Result<(), PeatError> {
        let state = self.state.read().unwrap();
        if !state.is_running {
            return Err(PeatError::NotRunning);
        }
        if state.connections.is_empty() {
            return Err(PeatError::InvalidState);
        }

        log::info!("Triggering sync with {} peers", state.connections.len());

        // TODO: Implement actual sync via peat-btle
        drop(state);
        let mut state = self.state.write().unwrap();
...
// ============================================================================
// PeatMesh Bindings - Centralized Peer & Document Management
// ============================================================================

use peat_btle::peat_mesh::{PeatMesh as RustPeatMesh, PeatMeshConfig as RustPeatMeshConfig};
use peat_btle::observer::{
    DisconnectReason as RustDisconnectReason, PeatEvent as RustPeatEvent,
};
use peat_btle::peer::PeatPeer as RustPeatPeer;
use peat_btle::sync::crdt::PeripheralType as RustPeripheralType;
...
impl From<RustPeatPeer> for MeshPeer {
    fn from(peer: RustPeatPeer) -> Self {
        MeshPeer {
...
impl From<RustPeatEvent> for MeshEvent {
    fn from(event: RustPeatEvent) -> Self {
        match event {
            RustPeatEvent::PeerDiscovered { peer } => MeshEvent::PeerDiscovered {
                peer: peer.into(),
            },
            RustPeatEvent::PeerConnected { node_id } => MeshEvent::PeerConnected {
                node_id: node_id.as_u32(),
            },
            RustPeatEvent::PeerDisconnected { node_id, reason } => MeshEvent::PeerDisconnected {
                node_id: node_id.as_u32(),
                reason: reason.into(),
            },
            RustPeatEvent::PeerLost { node_id } => MeshEvent::PeerLost {
                node_id: node_id.as_u32(),
            },
            RustPeatEvent::EmergencyReceived { from_node } => MeshEvent::EmergencyReceived {
                from_node: from_node.as_u32(),
            },
            RustPeatEvent::AckReceived { from_node } => MeshEvent::AckReceived {
                from_node: from_node.as_u32(),
            },
            RustPeatEvent::DocumentSynced {
                from_node,
                total_count,
            ...
            RustPeatEvent::MeshStateChanged {
                peer_count,
                connected_count,
...
impl peat_btle::observer::PeatObserver for SwiftMeshObserver {
    fn on_event(&self, event: RustPeatEvent) {
...
#[derive(uniffi::Object)]
pub struct PeatMeshWrapper {
    mesh: RustPeatMesh,
}

#[uniffi::export]
impl PeatMeshWrapper {
    ...
    pub fn new(
        ...
        let config = RustPeatMeshConfig::new(PeatNodeId::new(node_id), &callsign, &mesh_id)
            .with_peripheral_type(peripheral_type.into());

        Arc::new(PeatMeshWrapper {
            mesh: RustPeatMesh::new(config),
        })
    }
    ...
    pub fn with_defaults(node_id: u32, callsign: String) -> Arc<Self> {
        let config = RustPeatMeshConfig::new(
            PeatNodeId::new(node_id),
            &callsign,
            DEFAULT_MESH_ID,
        );
    ...
    pub fn on_incoming_connection(
        ...
        self.mesh
            .on_incoming_connection(&identifier, PeatNodeId::new(node_id), now_ms)
    }
...
mod tests {
    ...
    fn test_create_adapter() {
        init_logging();
        let adapter = PeatAdapter::new(0xDEADBEEF).unwrap();
    ...
    #[test]
    fn test_peat_mesh_wrapper() {
        let mesh = PeatMeshWrapper::with_defaults(0x12345678, "TEST".to_string());
