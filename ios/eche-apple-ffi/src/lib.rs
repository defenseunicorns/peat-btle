...
//! UniFFI bindings for eche-btle on Apple platforms
//!
//! This crate provides Swift bindings for the Eche BLE library using UniFFI.

use std::collections::HashMap;
use std::sync::{Arc, Mutex, RwLock};
use std::time::SystemTime;

use eche_btle::config::{BleConfig, BlePhy as EcheBlePhy, DiscoveryConfig, MeshConfig};
use eche_btle::platform::apple::CoreBluetoothAdapter;
use eche_btle::platform::BleAdapter;
use eche_btle::{NodeId as EcheNodeId, DEFAULT_MESH_ID};
...
pub fn init_logging() {
    let _ = env_logger::builder()
        ...
        .try_init();
    log::info!("eche-apple-ffi initialized");
}
...
pub struct ParsedDeviceName {
    /// Mesh ID (None for legacy ECHE- format)
    pub mesh_id: Option<String>,
    /// Node ID
    pub node_id: u32,
}

/// Parse an Eche device name to extract mesh ID and node ID
///
/// Supports both formats:
/// - New: `ECHE_<MESH_ID>-<NODE_ID>` (e.g., "ECHE_DEMO-12345678")
/// - Legacy: `ECHE-<NODE_ID>` (e.g., "ECHE-12345678")
#[uniffi::export]
pub fn parse_eche_device_name(name: String) -> Option<ParsedDeviceName> {
...
/// Generate an Eche device name for advertising
#[uniffi::export]
pub fn generate_eche_device_name(mesh_id: String, node_id: u32) -> String {
    let config = MeshConfig::new(mesh_id);
    config.device_name(EcheNodeId::new(node_id))
}
...
impl From<EcheBlePhy> for BlePhy {
    fn from(phy: EcheBlePhy) -> Self {
        match phy {
            EcheBlePhy::Le1M => BlePhy::Le1M,
            EcheBlePhy::Le2M => BlePhy::Le2M,
            EcheBlePhy::LeCodedS2 | EcheBlePhy::LeCodedS8 => BlePhy::LeCoded,
        }
    }
}
...
/// Error types for the Eche adapter
...
impl From<eche_btle::error::BleError> for EcheError {
    fn from(err: eche_btle::error::BleError) -> Self {
        match err {
            eche_btle::error::BleError::NotSupported(_) => EcheError::BluetoothUnavailable,
            eche_btle::error::BleError::ConnectionFailed(msg) => {
                EcheError::ConnectionFailed { reason: msg }
            }
            eche_btle::error::BleError::Timeout => EcheError::Timeout,
            eche_btle::error::BleError::PlatformError(msg) => EcheError::Internal { message: msg },
            _ => EcheError::Internal {
...
/// Main Eche BLE adapter interface
#[derive(uniffi::Object)]
pub struct EcheAdapter {
    node_id: u32,
    mesh_id: String,
...
impl EcheAdapter {
    ...
    #[uniffi::constructor]
    pub fn new(node_id: u32) -> Result<Arc<Self>, EcheError> {
        Self::with_mesh_id(node_id, DEFAULT_MESH_ID.to_string())
    }

    /// Create a new adapter with a specific mesh ID
    #[uniffi::constructor]
    pub fn with_mesh_id(node_id: u32, mesh_id: String) -> Result<Arc<Self>, EcheError> {
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .worker_threads(2)
            .enable_all()
            .build()
            .map_err(|e| EcheError::Internal {
                message: format!("Failed to create runtime: {}", e),
            })?;

        log::info!(
            "Creating EcheAdapter with node ID: {:08X}, mesh ID: {}",
            node_id,
            mesh_id
        );

        Ok(Arc::new(EcheAdapter {
            node_id,
            mesh_id,
    ...
    pub fn get_device_name(&self) -> String {
        ...
        config.device_name(EcheNodeId::new(self.node_id))
    }
    ...
    pub fn start(&self) -> Result<(), EcheError> {
        let mut state = self.state.write().unwrap();
        if state.is_running {
            return Err(EcheError::AlreadyInitialized);
        }

        log::info!("Starting EcheAdapter...");

        // Create the CoreBluetooth adapter
        let adapter = CoreBluetoothAdapter::new().map_err(|e| EcheError::Internal {
        ...
        *self.adapter.write().unwrap() = Some(adapter);
        ...
        log::info!("EcheAdapter started successfully");
        Ok(())
    }

    /// Stop the adapter
    pub fn stop(&self) -> Result<(), EcheError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }

        log::info!("Stopping EcheAdapter...");
    ...
    /// Start scanning for Eche peers
    pub fn start_discovery(&self) -> Result<(), EcheError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }
    ...
    /// Stop scanning
    pub fn stop_discovery(&self) -> Result<(), EcheError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }
    ...
    /// Start advertising as an Eche node
    pub fn start_advertising(&self) -> Result<(), EcheError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }
    ...
    /// Stop advertising
    pub fn stop_advertising(&self) -> Result<(), EcheError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }
    ...
    pub fn connect(&self, identifier: String) -> Result<(), EcheError> {
        ...
        let node_id = peer
            ...
            .ok_or_else(|| EcheError::ConnectionFailed {
        ...
        let adapter_guard = self.adapter.read().unwrap();
        if let Some(adapter) = adapter_guard.as_ref() {
            let eche_node_id = EcheNodeId::new(node_id);
            self.runtime
                .block_on(async { adapter.connect(&eche_node_id).await })
                .map_err(|e| EcheError::ConnectionFailed {
                    reason: e.to_string(),
                })?;
        }
    ...
    pub fn disconnect(&self, peer_id: u32) -> Result<(), EcheError> {
        let mut state = self.state.write().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }
        ...
        let adapter_guard = self.adapter.read().unwrap();
        if let Some(adapter) = adapter_guard.as_ref() {
            let node_id = EcheNodeId::new(peer_id);
            self.runtime
    ...
    pub fn send_data(&self, peer_id: u32, data: Vec<u8>) -> Result<(), EcheError> {
        let state = self.state.read().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }
        if !state.connections.contains_key(&peer_id) {
            return Err(EcheError::ConnectionFailed {
    ...
    pub fn broadcast_data(&self, data: Vec<u8>) -> Result<(), EcheError> {
        let state = self.state.read().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }
    ...
    pub fn trigger_sync(&self) -> Result<(), EcheError> {
        let state = self.state.read().unwrap();
        if !state.is_running {
            return Err(EcheError::NotRunning);
        }
        if state.connections.is_empty() {
            return Err(EcheError::InvalidState);
        }

        log::info!("Triggering sync with {} peers", state.connections.len());

        // TODO: Implement actual sync via eche-btle
        drop(state);
        let mut state = self.state.write().unwrap();
...
// ============================================================================
// EcheMesh Bindings - Centralized Peer & Document Management
// ============================================================================

use eche_btle::eche_mesh::{EcheMesh as RustEcheMesh, EcheMeshConfig as RustEcheMeshConfig};
use eche_btle::observer::{
    DisconnectReason as RustDisconnectReason, EcheEvent as RustEcheEvent,
};
use eche_btle::peer::EchePeer as RustEchePeer;
use eche_btle::sync::crdt::PeripheralType as RustPeripheralType;
...
impl From<RustEchePeer> for MeshPeer {
    fn from(peer: RustEchePeer) -> Self {
        MeshPeer {
...
impl From<RustEcheEvent> for MeshEvent {
    fn from(event: RustEcheEvent) -> Self {
        match event {
            RustEcheEvent::PeerDiscovered { peer } => MeshEvent::PeerDiscovered {
                peer: peer.into(),
            },
            RustEcheEvent::PeerConnected { node_id } => MeshEvent::PeerConnected {
                node_id: node_id.as_u32(),
            },
            RustEcheEvent::PeerDisconnected { node_id, reason } => MeshEvent::PeerDisconnected {
                node_id: node_id.as_u32(),
                reason: reason.into(),
            },
            RustEcheEvent::PeerLost { node_id } => MeshEvent::PeerLost {
                node_id: node_id.as_u32(),
            },
            RustEcheEvent::EmergencyReceived { from_node } => MeshEvent::EmergencyReceived {
                from_node: from_node.as_u32(),
            },
            RustEcheEvent::AckReceived { from_node } => MeshEvent::AckReceived {
                from_node: from_node.as_u32(),
            },
            RustEcheEvent::DocumentSynced {
                from_node,
                total_count,
            ...
            RustEcheEvent::MeshStateChanged {
                peer_count,
                connected_count,
...
impl eche_btle::observer::EcheObserver for SwiftMeshObserver {
    fn on_event(&self, event: RustEcheEvent) {
        self.callback.on_event(event.into());
    }
}

/// Main EcheMesh wrapper for UniFFI
#[derive(uniffi::Object)]
pub struct EcheMeshWrapper {
    mesh: RustEcheMesh,
}

#[uniffi::export]
impl EcheMeshWrapper {
    ...
    pub fn new(
        ...
        let config = RustEcheMeshConfig::new(EcheNodeId::new(node_id), &callsign, &mesh_id)
            .with_peripheral_type(peripheral_type.into());

        Arc::new(EcheMeshWrapper {
            mesh: RustEcheMesh::new(config),
        })
    }
    ...
    pub fn with_defaults(node_id: u32, callsign: String) -> Arc<Self> {
        let config = RustEcheMeshConfig::new(
            EcheNodeId::new(node_id),
            &callsign,
            DEFAULT_MESH_ID,
        );
    ...
    /// Returns the peer if it's a valid Eche peer, None otherwise
    ...
    pub fn on_incoming_connection(
        ...
        self.mesh
            .on_incoming_connection(&identifier, EcheNodeId::new(node_id), now_ms)
    }
...
mod tests {
    ...
    fn test_create_adapter() {
        init_logging();
        let adapter = EcheAdapter::new(0xDEADBEEF).unwrap();
    ...
    #[test]
    fn test_eche_mesh_wrapper() {
        let mesh = EcheMeshWrapper::with_defaults(0x12345678, "TEST".to_string());
