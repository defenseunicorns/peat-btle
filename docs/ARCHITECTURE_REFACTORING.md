# HIVE / hive-btle Architecture Refactoring Plan
...
> 4. **hive-btle** → Transport only; `standalone` feature adds hive-lite dep
...
The current hive-btle library conflates two different responsibilities:
...
This has caused bugs (chat not syncing) and architectural confusion. This document proposes a clear separation of concerns between HIVE (protocol layer) and hive-btle (transport layer).
...
3. **Architectural confusion** - Unclear what hive-btle should know about vs HIVE
...
In `HiveBtle.kt`, the `buildSyncDocumentForPeer()` function has two paths:
...
├─────────────────────────────────────────────────────────────────┤
│  ✗ HiveDocument format (counter, peripheral, emergency, chat)   │
...
### What hive-btle Currently Defines

| Component | Location | Problem |
|-----------|----------|---------|
| `HiveDocument` | `src/document.rs` | Application-level structure in transport |
...
| `HierarchyLevel` | `src/hive_mesh.rs` | Hierarchy is HIVE concept |
...
- Needs lightweight CRDTs in hive-btle
...
- hive-btle should just move bytes
...
| hive-btle | BLE transport | **NO** - just moves bytes |
...
```rust
/// Bridge between HIVE documents and hive-btle transport
pub trait BleBridge {
...
- [ ] Update hive-btle README to clarify scope
...
- [ ] Test HIVE → hive-btle → HIVE round-trip
...
**Context**: The current hive-btle ChatCRDT was added for WearTAK use. HIVE core ontology doesn't include messaging.
...
### Q3: What happens to existing hive-btle users?
...
**Current**: hive-btle has ChatCRDT in standalone mode
...
| 2026-01-25 | **hive-lite as separate repo** | Leaf crate with no HIVE deps; reusable by hive-btle, hive-lora, future transports | Kit, Codex |
...
| 2026-01-25 | **ChatCRDT deprecated** | Full chat doesn't fit hive-lite; will be removed or feature-gated in hive-btle | Kit, Codex |
...
- [HIVE ADR-039: HIVE-BTLE Mesh Transport](../hive/docs/adr/039-hive-btle.md)
- [hive-btle Issue cc953d6: Delta Sync Integration](https://app.radicle.xyz/nodes/seed.radicle.garden/rad:z458mp9Um3AYNQQFMdHaNEUtmiohq/issues/cc953d6)

---

## Appendix A: Current hive-btle Document Format

For reference, the current `HiveDocument` structure that would be deprecated for HIVE integration:
...
Wire format markers:
- `0xAA` - Unencrypted HiveDocument
- `0xAE` - Encrypted HiveDocument
...
hive-lite will be a **separate repository** (`hive-lite`), not a workspace member of hive-btle or hive. This enables:

1. **Reuse by multiple transports**: hive-btle, hive-lora (future), ESP-NOW
...
```
              ...
              ▼            ▼            ▼
        ┌──────────┐ ┌──────────┐ ┌──────────┐
        │hive-schema│ │hive-btle │ │hive-lora │
        └──────────┘ │(transport)│ │ (future) │
                     └─────┬─────┘ └────┬─────┘
                           │            │
                           │ optional   │ optional
...
### hive-btle Integration

```toml
# hive-btle/Cargo.toml
[features]
...
The translation layer lives in **hive-protocol**, not hive-lite or hive-btle.
...
### Phase 1: Integrate hive-lite into hive-btle

**Owner**: hive-btle Team
