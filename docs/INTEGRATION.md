# hive-btle Integration Guide

**Version**: hive-btle v0.1.0-rc26+
**Date**: 2026-01-26

## Architecture Overview

hive-btle is a **transport-only** library. It handles:
...
hive-btle does **NOT** handle application-layer protocols. For tactical messaging (CannedMessage, etc.), use **hive-lite** as a separate dependency.

```
┌─────────────────────────────────────────────────────────────┐
│                    Your Application                          │
├─────────────────────────────────────────────────────────────┤
│  hive-lite (optional)      │    hive-btle (required)        │
...
class MyActivity : AppCompatActivity(), HiveMeshListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        hiveBtle = HiveBtle(context = this, meshId = "DEMO")
        hiveBtle.init()
    ...
    override fun onDecryptedData(peer: HivePeer?, data: ByteArray) {
        ...
        when (data[0]) {
            ...
            0xAA.toByte() -> { /* HiveDocument - handled internally */ }
...
| Marker | Name | Handler |
|--------|------|---------|
| `0xAE` | Encrypted | hive-btle decrypts, passes inner data |
| `0xAF` | App-layer | Passed to `onDecryptedData`, app decodes |
| `0xAA` | HiveDocument | Processed internally by hive-btle |
| `0xB2` | DeltaDocument | Processed internally by hive-btle |
...
```kotlin
// Kotlin - using HiveMesh for encryption
...
```rust
// Rust - using HiveMesh for encryption
...
// Encrypt and send via hive-btle
...
|-----------|---------------|
| **hive-btle** | BLE transport, encryption, mesh sync, peer management |
| **hive-lite** | CannedMessage primitives, CRDT types (optional) |
| **Your App** | Message encoding/decoding, business logic, UI |

This separation ensures:
- hive-btle remains lightweight and transport-focused
