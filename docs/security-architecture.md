...
hive-btle implements a layered security model designed for tactical mesh networks operating on resource-constrained devices. This document covers the architecture decisions, trade-offs, and known limitations.
...
let mut key = [0u8; 32];
hkdf.expand(b"HIVE-BTLE-mesh-encryption-v1", &mut key);
```
...
struct PeerManager {
    peers: BTreeMap<NodeId, HivePeer>,      // Known peers
