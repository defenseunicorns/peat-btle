...
Starting with rc.28, hive-btle uses [UniFFI](https://mozilla.github.io/uniffi-rs/) for cross-language bindings instead of manual JNI. This provides:
...
// From genesis
val mesh = HiveMesh.createFromGenesis(genesis, identity, "ANDROID")
```

**After:**
```kotlin
// Factory method with peripheral type
val mesh = HiveMesh.newWithPeripheral(
    nodeId.toUInt(),
    "ANDROID",
    meshId,
...
val mesh = HiveMesh.newFromGenesis("ANDROID", identity, genesis)
```
...
## HiveBtle Integration

The `HiveBtle` class in `com.revolveteam.hive` has been updated to use UniFFI internally. If you're using `HiveBtle` directly, the public API remains largely the same - the changes are internal.

Key internal changes in HiveBtle:
- Removed `nativeInit`/`nativeShutdown` calls
- HiveMesh created via UniFFI factory methods
...
Ensure the native library (`libhive_btle.so`) is included in your APK's `jniLibs` folder for the correct ABI (arm64-v8a, armeabi-v7a, x86_64).
...
uniffi-bindgen generate \
    --library target/aarch64-linux-android/release/libhive_btle.so \
    --language kotlin \
...
Contact the hive-btle team or open an issue on Radicle.
