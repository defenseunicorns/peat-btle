...
hive-btle currently operates on a **shared secret model**: if you know the 32-byte mesh secret, you're a member. This creates a binary trust boundary with no gradations:
...
| **Mesh Detection** | Adversary maps all HIVE networks in area | High - cleartext ads |
...
As hive-btle moves toward production deployment with ATAK/WearTAK integration, the stakes increase. A mesh network carrying tactical situational awareness data requires trust guarantees beyond "you have the password."
...
│ Complete Local Name: 6 bytes (0x05 0x09 "HIVE")                 │
│ Service Data:       22 bytes                                    │
│   └─ Length:        1 byte  (0x15 = 21)                         │
│   └─ Type:          1 byte  (0x16 = Service Data)               │
│   └─ UUID:          2 bytes (0x7AF4 = HIVE service)             │
│   └─ Version:       1 byte  (0x01)                              │
...
fn derive_beacon_key(mesh_secret: &[u8; 32], time_slot: u64) -> [u8; 32] {
    let mut hasher = blake3::Hasher::new_derive_key("HIVE-beacon-v1");
...
fn derive_epoch_key(
    ...
    let mut hasher = blake3::Hasher::new_derive_key("HIVE-epoch-key-v1");
...
| Node ID derivation migration | 1d | Update `HiveMeshConfig` |
...
| Android advertisement update | 1d | `HiveBtle.kt` |
| iOS advertisement update | 1d | `HiveBLE.swift` |
...
**Acceptance Criteria**:
- [ ] Device name is generic "HIVE" (no identifiers)
...
| Leader rotation initiation | 1d | `HiveMesh::initiate_rotation()` |
...
hive-btle is the BLE transport layer within the broader HIVE (Hierarchical Intelligence for Versatile Entities) framework. This trust architecture must integrate seamlessly with HIVE's existing security model while supporting standalone operation.
...
hive-btle maps these credentials as follows:

| HIVE Credential | hive-btle Mapping | Usage |
|-----------------|-------------------|-------|
...
When hive-btle operates as a transport under hive-protocol:

```rust
impl HiveBtleTransport {
    ...
    pub fn from_hive_credentials(
        ...
        Ok(Self::new(HiveMeshConfig::new(
...
- hive-btle provides transport-layer trust; hive-protocol provides application-layer trust

#### Mode 2: Standalone Operation

When hive-btle operates independently (e.g., pure BLE mesh without HIVE backend):
...
impl HiveMeshConfig {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        let mut config = Self::new(
            identity.node_id(),
            &env::var("HIVE_BTLE_CALLSIGN").unwrap_or_else(|_| "HIVE".to_string()),
            &mesh_id,
...
│                              ▼                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │           hive-btle Transport Layer (This ADR)           │   │
...
When a node has both hive-btle identity and hive-protocol identity:

```rust
/// Bind hive-btle identity to hive-protocol certificate
pub struct IdentityBinding {
    /// hive-btle Ed25519 public key
    btle_pubkey: [u8; 32],
...
When hive-btle is managed by hive-protocol:
...
pub trait MembershipDelegate {
    ...
    /// Called when hive-btle wants to revoke a node
...
- Coordinate revocations across HIVE and hive-btle layers
...
pub enum RotationSource {
    /// Initiated by hive-btle (e.g., local compromise detected)
    BtleLocal,
