...
**Status**: Draft
**Date**: 2026-02-01
**Context**: Enable external crates (hive-lite, app-layer) to register custom CRDT document types that sync through hive-btle's delta mechanism.

## Problem

Currently, hive-btle has hardcoded document types (`Peripheral`, `EmergencyEvent`, `ChatCRDT`) in `HiveDocument`. External crates like hive-lite define their own CRDT types (e.g., `CannedMessageEvent`) that need to sync across the mesh.
...
When an app-layer document is included in a HiveDocument:
...
- Add `DocumentType` trait and `DocumentRegistry` to hive-btle
- Add `AppOperation` to delta_document.rs
- Expose registry via `HiveMesh::document_registry()`
...
### A. Hardcode CannedMessage in hive-btle
**Rejected**: Creates circular dependency, violates hive-btle's standalone design.
...
- [hive-lite Codex.md](../../../hive-lite/Codex.md) - CannedMessage wire format
