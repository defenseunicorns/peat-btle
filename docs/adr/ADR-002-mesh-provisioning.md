...
This ADR defines how hive-btle meshes are created, how nodes are provisioned to join, and how trust is established.

**Key Architectural Principle**: hive-btle is **fully self-sufficient**. It has complete, independent security - identity, provisioning, key management, membership control. It does NOT depend on HIVE or any external system.

HIVE (the larger framework) may **consume** hive-btle as a BLE transport, but hive-btle never defers to or requires HIVE.
...
│                                                                 │
│  HIVE consumes hive-btle as a transport layer.                 │
...
│         └─────────────────────────────────────┘                │
│                                                                 │
│  Config API allows external systems (like HIVE) to provide:    │
│  • Pre-generated identity keys                                  │
│  • Pre-shared mesh secrets                                      │
│  • Membership rosters                                           │
│                                                                 │
│  But hive-btle owns and manages all of this itself.            │
└─────────────────────────────────────────────────────────────────┘
```
...
hive-btle must work in these scenarios:

| Scenario | Description |
|----------|-------------|
| **Pure Standalone** | hive-btle only, no HIVE framework. Sensors + phones form mesh. |
| **HIVE Transport** | HIVE uses hive-btle for BLE comms, may provide keys via config. |
| **Mixed Deployment** | Some nodes HIVE-managed, some standalone hive-btle only. |

In ALL cases, hive-btle:
...
When HIVE consumes hive-btle, it may:
- Provide pre-generated identity via `HiveMeshConfig`
- Provide mesh secrets derived from HIVE's MLS
- Sync membership rosters via hive-btle's API

But hive-btle never "asks" HIVE for anything. It's a library, not a client.
