# HIVE-BTLE API Reference

This document provides a comprehensive guide to the main types, traits, and APIs in the `hive-btle` crate.
...
- [Entry Points](#entry-points)
  - [HiveMesh (High-Level)](#hivemesh-high-level)
  - [BluetoothLETransport (Low-Level)](#bluetoothletransport-low-level)
- [Configuration](#configuration)
  - [BleConfig](#bleconfig)
  - [HiveMeshConfig](#hivemeshconfig)
...
- [Events and Observers](#events-and-observers)
  - [HiveEvent](#hiveevent)
  - [HiveObserver Trait](#hiveobserver-trait)
...
`hive-btle` provides two main entry points:

| Entry Point | Use Case | Complexity |
|-------------|----------|------------|
| `HiveMesh` | Full mesh management with peers, sync, events | High-level |
| `BluetoothLETransport` | Raw BLE transport for custom implementations | Low-level |

For most applications, use `HiveMesh`. It handles peer discovery, document synchronization, encryption, and event notification automatically.
...
Bitflags indicating node capabilities, advertised in the HIVE beacon.

```rust
use hive_btle::capabilities;

let caps = capabilities::LITE_NODE        // HIVE-Lite node
...
Available flags:
- `LITE_NODE` (0x0001): HIVE-Lite node (minimal state)
...
`HiveMesh` is the main facade for HIVE BLE mesh operations. It composes peer management, document sync, and observer notifications.
...
mesh.get_peers()         // Vec<HivePeer> of all peers
mesh.get_connected_peers() // Vec<HivePeer> of connected peers
mesh.get_peer(node_id)   // Option<HivePeer>
...
```rust
use hive_btle::{BluetoothLETransport, BleConfig, NodeId, MeshTransport};
...
```rust
use hive_btle::{BleConfig, NodeId, PowerProfile, BlePhy};

// Default configuration
let config = BleConfig::new(NodeId::new(0x12345678));

// HIVE-Lite optimized (low power, leaf node)
let config = BleConfig::hive_lite(NodeId::new(0x12345678));
```
...
Configuration for the high-level `HiveMesh` facade.

```rust
use hive_btle::{HiveMeshConfig, NodeId};
use hive_btle::sync::crdt::PeripheralType;
...
```rust
// At creation
let config = HiveMeshConfig::new(node_id, "ALPHA", "DEMO")
...
```rust
use hive_btle::observer::HiveEvent;

match event {
    // Peer lifecycle
    HiveEvent::PeerDiscovered { peer } => { /* new peer found */ }
    HiveEvent::PeerConnected { node_id } => { /* peer connected */ }
    HiveEvent::PeerDisconnected { node_id, reason } => { /* peer disconnected */ }
    HiveEvent::PeerLost { node_id } => { /* peer timed out */ }

    // Mesh events
    HiveEvent::EmergencyReceived { from_node } => { /* emergency! */ }
    HiveEvent::AckReceived { from_node } => { /* peer ACKed */ }
    HiveEvent::DocumentSynced { from_node, total_count } => { /* sync complete */ }
    HiveEvent::MeshStateChanged { peer_count, connected_count } => { /* state change */ }

    // Per-peer E2EE
    HiveEvent::PeerE2eeEstablished { peer_node_id } => { /* E2EE ready */ }
    HiveEvent::PeerE2eeClosed { peer_node_id } => { /* E2EE ended */ }
    HiveEvent::PeerE2eeMessageReceived { from_node, data } => { /* encrypted msg */ }

    // Security
    HiveEvent::SecurityViolation { kind, source } => { /* security issue */ }
}
```
...
```rust
use hive_btle::observer::CollectingObserver;
...
assert!(events.iter().any(|e| matches!(e, HiveEvent::EmergencyReceived { .. })));
```
...
```toml
[dependencies]
hive-btle = { version = "0.1", features = ["linux", "coded-phy"] }
```
...
```rust
use hive_btle::{
    HIVE_SERVICE_UUID,       // 128-bit service UUID
    HIVE_SERVICE_UUID_16BIT, // 16-bit short form (0xF47A)
