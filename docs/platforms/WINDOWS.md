...
This guide covers integrating `hive-btle` into Windows applications using the WinRT Bluetooth APIs.
...
```toml
[dependencies]
hive-btle = { version = "0.1", features = ["windows"] }
...
use hive_btle::{BleConfig, NodeId, MeshTransport};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("HIVE BLE running on Windows...");
...
// Set filter for HIVE service UUID
...
service_uuids.Append(HIVE_SERVICE_GUID)?;
...
let advertisement = BluetoothLEAdvertisement::new()?;
advertisement.SetLocalName(&HSTRING::from("HIVE_DEMO-12345678"))?;
...
writer.WriteUInt16(HIVE_SERVICE_UUID_16BIT)?;
...
async fn connect_to_device(address: u64) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Get HIVE service
    let services = device.GetGattServicesForUuidAsync(HIVE_SERVICE_GUID)?.await?;
...
async fn create_gatt_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = GattServiceProvider::CreateAsync(HIVE_SERVICE_GUID)?.await?;
...
## High-Level Integration with HiveMesh
...
impl HiveObserver for WindowsObserver {
...
fn setup_logging() {
    Builder::new()
        ...
        .filter_module("hive_btle", LevelFilter::Trace)
        .init();
}
```
