...
```rust
let config = HiveMeshConfig::new(node_id, callsign, mesh_id)
    .with_encryption(secret);
let mesh = HiveMesh::new(config);
```
...
```rust
let mut mesh = HiveMesh::new(config);
...
```rust
// All known peers
let all_peers: Vec<HivePeer> = mesh.get_peers();

// Connected peers only
let connected: Vec<HivePeer> = mesh.get_connected_peers();
...
impl HiveObserver for MyObserver {
    fn on_event(&self, event: HiveEvent) {
        match event {
            ...
            HiveEvent::PeerDisconnected { node_id, reason } => {
                println!("Disconnected: {:08X} ({:?})", node_id.as_u32(), reason);
            }
            HiveEvent::PeerLost { node_id } => {
                println!("Lost (stale): {:08X}", node_id.as_u32());
            }

            // Mesh events
            HiveEvent::MeshStateChanged { peer_count, connected_count } => {
                println!("Mesh: {}/{} connected", connected_count, peer_count);
            }

            // Sync events
            HiveEvent::DocumentSynced { from_node, total_count } => {
            ...
            HiveEvent::AckReceived { from_node } => {
...
```rust
use hive_btle::security::EncryptionError;
...
let mesh = HiveMesh(config: config)

// CoreBluetooth callbacks → HiveMesh
...
import com.hive.btle.HiveMeshConfig
...
val mesh = HiveMesh(config)

// BluetoothGattCallback → HiveMesh
...
let config = if secure_mode_required {
    HiveMeshConfig::new(node_id, callsign, mesh_id)
        .with_encryption(secret)
} else {
...
mod tests {
    ...
    fn create_test_mesh(node_id: u32, secret: Option<[u8; 32]>) -> HiveMesh {
        let mut config = HiveMeshConfig::new(
            NodeId::new(node_id),
            &format!("TEST-{}", node_id),
            "TEST"
        );
