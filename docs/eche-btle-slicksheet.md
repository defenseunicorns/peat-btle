...
ECHE-BTLE is a Bluetooth Low Energy mesh networking library that enables secure, resilient communication between devices when traditional infrastructure is unavailable. Built for tactical and field operations where connectivity cannot be guaranteed.
...
│                    Application Layer                     │
├─────────────────────────────────────────────────────────┤
│  EcheMesh API  │  Chat CRDT  │  Location  │  Emergency  │
├─────────────────────────────────────────────────────────┤
...
```toml
[dependencies]
eche-btle = { version = "0.1.0", features = ["linux"] }
```
...
val mesh = EcheMesh.createWithIdentity(nodeId, "ALPHA-1", "SQUAD", secret)
...
*ECHE-BTLE: Mesh when it matters.*
