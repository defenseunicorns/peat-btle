...
class GattCallbackProxy(private val connectionId: Long) : BluetoothGattCallback() {
    ...
    override fun onMtuChanged(gatt: BluetoothGatt, mtu: Int, status: Int) {
        ...
        // Clear GATT cache before discovery to avoid stale service lists.
        // Android caches GATT services per MAC address and returns stale results
        // if the peripheral's services changed since the last connection.
        refreshGattCache(gatt)
     ...
     */
    /**
     * Clear Android's GATT service cache via the hidden BluetoothGatt.refresh() API.
     *
     * Android aggressively caches GATT service discovery results per device MAC address.
     * If a peripheral's services change between connections (e.g. different firmware,
     * different app registering services), Android returns the stale cached list instead
     * of performing fresh discovery. This causes "service not found" errors.
     *
     * BluetoothGatt.refresh() is a @hide API that has been stable since Android 6.0.
     * It's used by virtually all production BLE apps (nRF Connect, etc.).
     */
    private fun refreshGattCache(gatt: BluetoothGatt) {
        try {
            val method = gatt.javaClass.getMethod("refresh")
            val result = method.invoke(gatt) as? Boolean ?: false
            Log.d(TAG, "[$connectionId] GATT cache refresh: $result")
        } catch (e: Exception) {
            Log.w(TAG, "[$connectionId] GATT cache refresh not available: ${e.message}")
        }
    }
