...
//!
//! This binary runs on macOS and acts as a HIVE mesh node that:
...
struct TestState {
    ...
    /// Track HIVE peripherals by their CoreBluetooth identifier (for devices without node ID in name)
    hive_peripherals: std::collections::HashMap<String, HivePeripheral>,
}
...
/// Track a discovered HIVE peripheral before we know its node ID
struct HivePeripheral {
    identifier: String,
    name: Option<String>,
    last_seen: u64,
    last_rssi: i16,
...
impl TestState {
    fn new(log_path: Option<&str>) -> Self {
        ...
        Self {
            ...
            hive_peripherals: std::collections::HashMap::new(),
        }
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    adapter.set_discovery_callback(Some(Arc::new(move |device: DiscoveredDevice| {
        // In debug mode, log ALL devices; otherwise only HIVE nodes
                ...
                // Log discovery
                {
                    ...
                    if let Some(nid) = node_id_opt {
                        ...
                        if let Some(name) = &device_clone.name {
                            // Format: HIVE_<MESH>-<CALLSIGN>-<SHORT_ID> or HIVE-<NODE_ID>
                            if let Some(rest) = name.strip_prefix("HIVE_") {
                                // HIVE_WEARTAK-RANGER-8DD4
    ...
    if debug_scan {
        ...
        log::info!("MODE: Passive - listening for HIVE advertisements");
    }
        ...
        tokio::select! {
            _ = interval.tick() => {
                ...
                if tick_count % 100 == 0 {
                    ...
                    // Log HIVE peripherals by identifier (devices without node ID in name)
                    let hive_count = state.hive_peripherals.len();
                    let connectable: Vec<_> = state.hive_peripherals.values()
                        .filter(|p| p.connection_state == ConnectionState::Discovered && now.saturating_sub(p.last_seen) < 10000)
                        .collect();
                    if hive_count > 0 {
                        log::info!("  HIVE peripherals: {} total, {} ready to connect", hive_count, connectable.len());
                ...
                // Attempt to connect to discovered HIVE peripherals every 5 seconds
                if tick_count % 50 == 25 {
                    ...
                    if let Some(identifier) = connectable {
                        log::info!("Attempting to connect to HIVE peripheral: {}...", &identifier[..8]);

                        // Mark as connecting
                        {
                            let mut state = test_state.write().await;
                            if let Some(p) = state.hive_peripherals.get_mut(&identifier) {
                ...
                while let Some(event) = adapter.try_recv_peripheral_event().await {
                    use hive_btle::platform::apple::PeripheralEvent;
                    match event {
                        PeripheralEvent::ServicesDiscovered { identifier, error } => {
                            if let Some(err) = error {
                                ...
                                // Discover characteristics for HIVE service
                        ...
                        PeripheralEvent::CharacteristicsDiscovered { identifier, service_uuid, error } => {
                            if let Some(err) = error {
                                ...
                                let node_info_uuids = [
                                    "F47A0001-58CC-4372-A567-0E02B2C3D479",  // Full HIVE format
                                    "0001",                                   // Short format
                        ...
                        PeripheralEvent::CharacteristicValueUpdated { identifier, characteristic_uuid, value, error } => {
                            if let Some(err) = error {
                                ...
                                // If this is sync_data (0003), it contains encrypted HIVE documents
                                    ...
                                    // Process through HiveMesh to decrypt and extract data
                                    let result = {
                                        let mesh_guard = mesh.read().await;
