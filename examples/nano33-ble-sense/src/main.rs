//! HIVE Sensor Beacon for Arduino Nano 33 BLE Sense
//!
//! Transmit-only HIVE node that:
//! - Advertises HIVE beacon
...
/// HIVE Service UUID (16-bit short form for advertising)
const HIVE_SERVICE_UUID_16: u16 = 0xF47A;

/// Device name in advertisements
const DEVICE_NAME: &str = "HIVE-SENSE";
...
async fn main(spawner: Spawner) {
    info!("HIVE Sensor Beacon starting...");
    ...
    // Create GATT server
    let server = unwrap!(HiveServer::new(sd));
    ...
    loop {
        ...
        let adv_data = LegacyAdvertisementBuilder::new()
            ...
            .services_16(
                ...
                &[ServiceUuid16::from_u16(HIVE_SERVICE_UUID_16)],
            )
        ...
        match peripheral::advertise_connectable(sd, adv, &config).await {
            Ok(conn) => {
                ...
                let res = gatt_server::run(&conn, &server, |event| match event {
                    HiveServerEvent::Hive(e) => match e {
                        HiveServiceEvent::DocumentWrite(data) => {
                            info!("Received document: {} bytes", data.len());
                            // TODO: Merge into local CRDT
                        }
                        HiveServiceEvent::SensorDataCccdWrite { notifications } => {
                            info!("Sensor notifications: {}", notifications);
                        }
                        HiveServiceEvent::DocumentCccdWrite { notifications } => {
