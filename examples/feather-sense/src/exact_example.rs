//! Exact copy of nrf-softdevice ble_advertise example
...
async fn main(spawner: Spawner) {
    ...
    // EXACT config from nrf-softdevice example
    ...
    // Blink 4 times before advertise
    for _ in 0..4 {
        led.set_high();
        Timer::after(Duration::from_millis(200)).await;
        led.set_low();
    ...
    unwrap!(peripheral::advertise(sd, adv, &adv_config).await);

    info!("Advertise completed!");

    // Continuous blink = success
    loop {
        led.set_high();
