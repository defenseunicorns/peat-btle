//! HIVE Mesh Node - Pure Rust BLE GATT Server for nRF52840
//!
//! Implements the HIVE GATT service for mesh networking with WearTAK.
//! Advertises as "HIVE-xxxx" with the HIVE service UUID.
...
// HIVE Service UUID 16-bit (for advertising)
const HIVE_SERVICE_UUID_16BIT: u16 = 0xF47A;
...
// GATT Server definition with HIVE service
#[gatt_server]
struct HiveServer {
...
async fn run_ble<C: Controller>(controller: C, mut led: Output<'_>) {
    ...
    let name_str = core::str::from_utf8(&name_bytes).unwrap_or("HIVE");
    ...
    info!("Creating HIVE GATT server...");

    // Use ManuallyDrop to prevent Drop from being called (we never return)
    let server = ManuallyDrop::new(HiveServer::new_with_config(GapConfig::Peripheral(PeripheralConfig {
        name: name_str,
    ...
    let _ = server.set(&server.hive_service.node_info, &build_node_info());
    ...
    let advertising = async {
        loop {
            match advertise_hive(&name_bytes, &mut peripheral, &server).await {
                Ok(conn) => {
...
async fn advertise_hive<'a, C: Controller>(
    name: &[u8; 9],
    peripheral: &mut Peripheral<'a, C, DefaultPacketPool>,
    server: &'a HiveServer<'a>,
    ...
    // 16-bit Service UUID (HIVE)
    adv_data[pos] = 3; pos += 1;
    adv_data[pos] = 0x03; pos += 1;
    adv_data[pos] = (HIVE_SERVICE_UUID_16BIT & 0xFF) as u8; pos += 1;
    adv_data[pos] = (HIVE_SERVICE_UUID_16BIT >> 8) as u8; pos += 1;
    ...
    info!("Advertising as '{}'...", core::str::from_utf8(name).unwrap_or("HIVE"));
    ...
    loop {
        match conn.next().await {
            ...
            GattConnectionEvent::Gatt { event } => {
                match &event {
                    GattEvent::Read(e) => {
                        if e.handle() == server.hive_service.node_info.handle {
                            info!("Read: node_info");
                        } else if e.handle() == server.hive_service.sync_state.handle {
                            info!("Read: sync_state");
                        } else if e.handle() == server.hive_service.status.handle {
                            info!("Read: status");
                        }
                    }
                    GattEvent::Write(e) => {
                        if e.handle() == server.hive_service.command.handle {
                            info!("Write: command = {:?}", e.data());
                        } else if e.handle() == server.hive_service.sync_data.handle {
...
async fn notification_task<P: PacketPool>(
    ...
    loop {
        ...
        if server.hive_service.status.notify(conn, &status).await.is_err() {
