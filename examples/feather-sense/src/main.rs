//! HIVE SOS Beacon for Adafruit Feather nRF52840 Sense
//!
...
/// HIVE service UUID 16-bit (matches Android HiveBtle.kt)
...
/// Build HIVE beacon service data (10 bytes)
...
async fn main(spawner: Spawner) {
    ...
    info!("HIVE-SENSE starting... NodeID: {:08X}", node_id);
    ...
    let server = HiveServer::new(sd).unwrap();
            ...
            Ok(Ok(conn)) => {
                ...
                let res = gatt_server::run(&conn, &server, |event| match event {
                    HiveServerEvent::Hive(e) => match e {
                        ...
                        HiveServiceEvent::EmergencyWrite(data) => {
                        ...
                        HiveServiceEvent::EmergencyCccdWrite { notifications } => {
