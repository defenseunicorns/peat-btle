...
pub trait BleAdapter: Send + Sync {
    ...
    /// Write data to a peer's GATT characteristic
    ///
    /// This is the low-level send primitive used by `MeshTransport::send_to()`
    /// to transmit data fragments over BLE.
    ///
    /// # Arguments
    /// * `peer_id` - Target peer node ID
    /// * `char_uuid` - GATT characteristic UUID to write to
    /// * `data` - Bytes to write (must fit within negotiated MTU)
