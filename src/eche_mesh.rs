...
//! EcheMesh - Unified mesh management facade
//!
//! This module provides the main entry point for Eche BLE mesh operations.
...
//! mesh.on_ble_discovered("device-uuid", Some("ECHE_DEMO-AABBCCDD"), -65, Some("DEMO"), now_ms);
...
use crate::observer::{DisconnectReason, EcheEvent, EcheObserver, SecurityViolationKind};
use crate::peer::{
    ConnectionStateGraph, EchePeer, FullStateCountSummary, IndirectPeer, PeerConnectionState,
...
/// Main facade for Eche BLE mesh operations
///
...
#[cfg(feature = "std")]
impl EcheMesh {
    /// Create a new EcheMesh instance
    pub fn new(config: EcheMeshConfig) -> Self {
        ...
        #[cfg(feature = "eche-lite-sync")]
        {
            ...
            log::info!("Auto-registered CannedMessageDocument (0xC0) for eche-lite sync");
        }
    ...
    /// Create a new EcheMesh with a cryptographic identity
    ///
    ...
    pub fn with_identity(config: EcheMeshConfig, identity: DeviceIdentity) -> Self {
    ...
    /// Create a new EcheMesh from genesis data
    ///
    ...
    pub fn from_genesis(
        ...
        let config = EcheMeshConfig::new(identity.node_id(), callsign, &genesis.mesh_id())
            .with_encryption(genesis.encryption_secret());

        Self::with_identity(config, identity)
    }

    /// Create a EcheMesh from persisted state.
    ///
    ...
    /// let mesh = EcheMesh::from_persisted(state, "SENSOR-01")?;
    /// ```
    #[cfg(feature = "std")]
    pub fn from_persisted(
        ...
        let mesh = if let Some(ref gen) = genesis {
            ...
            let config = EcheMeshConfig::new(identity.node_id(), callsign, "RESTORED");
            Self::with_identity(config, identity)
        };
        ...
        log::info!(
            "EcheMesh restored from persisted state: node_id={:08X}, known_peers={}",
            ...
            match &self.encryption_key {
                Some(key) => match key.decrypt_from_bytes(encrypted_payload) {
                    ...
                    Err(e) => {
                        ...
                        self.notify(EcheEvent::SecurityViolation {
    ...
    /// message parsing themselves (using eche-lite or other libraries).
    ///
    /// # Arguments
    ...
    pub fn get_relay_targets(&self, exclude_peer: Option<NodeId>) -> Vec<EchePeer> {
    ...
    pub fn process_relay_envelope(
        ...
        if !self.mark_message_seen(envelope.message_id, envelope.origin_node, now_ms) {
            ...
            self.notify(EcheEvent::DuplicateMessageDropped {
        ...
        if !envelope.can_relay() {
            self.notify(EcheEvent::MessageTtlExpired {
                origin_node: envelope.origin_node,
    ...
    fn process_delta_document_internal(
        ...
        for op in &delta.operations {
            ...
            match op {
                ...
                Operation::App(app_op) => {
                    ...
                    self.observers.notify(EcheEvent::app_document_received(
                        app_op.type_id,
        ...
        if is_emergency {
            self.notify(EcheEvent::EmergencyReceived {
                from_node: delta.origin_node,
            });
        } else if is_ack {
            self.notify(EcheEvent::AckReceived {
                from_node: delta.origin_node,
            });
        }
        ...
        if counter_changed {
            ...
            self.notify(EcheEvent::DocumentSynced {
                from_node: delta.origin_node,
                total_count,
            });
        }
        ...
        if relay_data.is_some() {
            ...
            self.notify(EcheEvent::MessageRelayed {
    ...
    pub fn close_peer_e2ee(&self, peer_node_id: NodeId) {
        ...
        if let Some(session_mgr) = sessions.as_mut() {
            ...
            self.notify(EcheEvent::PeerE2eeClosed { peer_node_id });
            log::info!(
    ...
    fn handle_key_exchange(&self, data: &[u8], now_ms: u64) -> Option<Vec<u8>> {
        ...
        if established {
            self.notify(EcheEvent::PeerE2eeEstablished {
    ...
    fn handle_peer_e2ee_message(&self, data: &[u8], now_ms: u64) -> Option<Vec<u8>> {
        ...
        match session_mgr.decrypt_from_peer(&msg, now_ms) {
            Ok(plaintext) => {
                ...
                self.notify(EcheEvent::PeerE2eeMessageReceived {
    ...
    ///
    /// ```ignore
    /// use eche_btle::registry::DocumentType;
    ///
    ...
    pub fn add_observer(&self, observer: Arc<dyn EcheObserver>) {
        self.observers.add(observer);
    }

    /// Remove an observer
    pub fn remove_observer(&self, observer: &Arc<dyn EcheObserver>) {
    ...
    pub fn send_emergency(&self, timestamp: u64) -> Vec<u8> {
        ...
        self.notify(EcheEvent::MeshStateChanged {
    ...
    ///
    /// Returns `Some(EchePeer)` if this is a new Eche peer on our mesh.
    pub fn on_ble_discovered(
        ...
        if is_new {
            self.notify(EcheEvent::PeerDiscovered { peer: peer.clone() });
    ...
    pub fn on_ble_connected(&self, identifier: &str, now_ms: u64) -> Option<NodeId> {
        ...
        self.notify(EcheEvent::PeerConnected { node_id });
        self.notify_mesh_state_changed();
        Some(node_id)
    }
    ...
    pub fn on_ble_disconnected(
        ...
        self.notify(EcheEvent::PeerDisconnected {
            node_id,
    ...
    pub fn on_peer_disconnected(&self, node_id: NodeId, reason: DisconnectReason) {
            ...
            .on_disconnected_by_node_id(node_id, reason)
        {
            ...
            self.notify(EcheEvent::PeerDisconnected { node_id, reason });
            self.notify_mesh_state_changed();
        }
    }
    ...
    pub fn on_incoming_connection(&self, identifier: &str, node_id: NodeId, now_ms: u64) -> bool {
        ...
        if is_new {
            if let Some(peer) = self.peer_manager.get_peer(node_id) {
                self.notify(EcheEvent::PeerDiscovered { peer });
            }
        }
    ...
    pub fn on_ble_data_received_anonymous(
        ...
        // Handle app-layer message (0xAF marker from eche-lite CannedMessages)
        ...
        if decrypted.first().copied() == Some(APP_LAYER_MARKER) {
            ...
            #[cfg(not(feature = "eche-lite-sync"))]
            {
                log::debug!("Ignoring 0xAF message (eche-lite-sync feature not enabled)");
            }
    ...
    pub fn tick(&self, now_ms: u64) -> Option<Vec<u8>> {
        ...
        if cleanup_elapsed >= self.config.peer_config.cleanup_interval_ms as u32 {
            ...
            for node_id in &removed {
                self.notify(EcheEvent::PeerLost { node_id: *node_id });
            }
            if !removed.is_empty() {
                self.notify_mesh_state_changed();
            }
            ...
            // Run connection graph maintenance (transition Disconnected -> Lost)
            {
                ...
                for node_id in newly_lost {
                    ...
                    if !removed.contains(&node_id) {
                        self.notify(EcheEvent::PeerLost { node_id });
                    }
                }
            }
        }
    ...
    fn notify(&self, event: EcheEvent) {
    ...
    //
    // These methods provide deduplication support for eche-lite CannedMessages.
...
mod tests {
    ...
    fn create_mesh(node_id: u32, callsign: &str) -> EcheMesh {
        let config = EcheMeshConfig::new(NodeId::new(node_id), callsign, "TEST");
        EcheMesh::new(config)
    }
    ...
    fn test_mesh_creation() {
        ...
        assert_eq!(mesh.device_name(), "ECHE_TEST-12345678");
    }
    ...
    fn test_peer_discovery() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::PeerDiscovered { .. })));
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::MeshStateChanged { .. })));
    }
    ...
    fn test_connection_lifecycle() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::PeerConnected { .. })));
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::PeerDisconnected { .. })));
    }
    ...
    fn test_emergency_flow() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::EmergencyReceived { .. })));
    }
    ...
    fn test_ack_flow() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::AckReceived { .. })));
    }

    #[test]
    fn test_tick_cleanup() {
        let config = EcheMeshConfig::new(NodeId::new(0x11111111), "ALPHA-1", "TEST")
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::PeerLost { .. })));
    }
    ...
    fn create_encrypted_mesh(node_id: u32, callsign: &str, secret: [u8; 32]) -> EcheMesh {
        let config =
            EcheMeshConfig::new(NodeId::new(node_id), callsign, "TEST").with_encryption(secret);
        EcheMesh::new(config)
    }
    ...
    fn test_peer_e2ee_full_handshake() {
        ...
        assert!(events1
            .iter()
            .any(|e| matches!(e, EcheEvent::PeerE2eeEstablished { .. })));
    ...
    fn test_peer_e2ee_close_session() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::PeerE2eeClosed { .. })));
    }
    ...
    fn create_strict_encrypted_mesh(node_id: u32, callsign: &str, secret: [u8; 32]) -> EcheMesh {
        let config = EcheMeshConfig::new(NodeId::new(node_id), callsign, "TEST")
    ...
    fn test_strict_mode_security_violation_event_includes_source() {
        ...
        if let Some(EcheEvent::SecurityViolation { source, .. }) = violation {
    ...
    fn create_relay_mesh(node_id: u32, callsign: &str) -> EcheMesh {
        let config = EcheMeshConfig::new(NodeId::new(node_id), callsign, "TEST").with_relay();
        EcheMesh::new(config)
    }
    ...
    fn test_process_relay_envelope_duplicate() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::DuplicateMessageDropped { .. })));
    }
    ...
    fn test_process_relay_envelope_ttl_expired() {
        ...
        assert!(events
            .iter()
            .any(|e| matches!(e, EcheEvent::MessageTtlExpired { .. })));
    }
    ...
    fn test_build_relay_document() {
        ...
        // The payload should be a valid EcheDocument
        let doc = crate::document::EcheDocument::decode(&envelope.payload);
        assert!(doc.is_some());
    }
