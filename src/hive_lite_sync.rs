...
//! Integration between hive-btle and hive-lite.
//!
//! This module provides [`DocumentType`] implementations for hive-lite types,
//! enabling them to sync through the hive-btle mesh using the extensible
//! document registry.
//!
//! ## Usage
//!
//! Enable the `hive-lite-sync` feature in your `Cargo.toml`:
//!
//! ```toml
//! [dependencies]
//! hive-btle = { version = "0.1", features = ["hive-lite-sync"] }
//! ```
//!
//! Then register the CannedMessage type with the mesh:
//!
//! ```ignore
//! use hive_btle::{HiveMesh, DocumentRegistry};
//! use hive_btle::hive_lite_sync::CannedMessageDocument;
//!
...
//! let event = hive_lite::CannedMessageAckEvent::new(
//!     hive_lite::CannedMessage::CheckIn,
//!     hive_lite::NodeId::new(my_node_id),
//!     None,
//!     timestamp_ms,
//! );
...
use hive_lite::{CannedMessageAckEvent, NodeId as HiveLiteNodeId};
...
///
/// This enables CannedMessage events to sync through the hive-btle mesh
...
impl CannedMessageDocument {
    ...
    pub fn ack(&mut self, node_id: u32, ack_timestamp: u64) -> bool {
        self.inner.ack(HiveLiteNodeId::new(node_id), ack_timestamp)
    }

    /// Check if a node has acknowledged.
    pub fn has_acked(&self, node_id: u32) -> bool {
        self.inner.has_acked(HiveLiteNodeId::new(node_id))
    }
        ...
        match op.op_code {
            op_code::ACK_UPDATE => {
                ...
                for _ in 0..num_acks {
                    ...
                    if self.inner.ack(HiveLiteNodeId::new(acker_node), ack_ts) {
                        changed = true;
                    }
                }
...
mod tests {
    ...
    fn test_document_encode_decode() {
        let event = CannedMessageAckEvent::new(
            ...
            HiveLiteNodeId::new(0xAABBCCDD),
            Some(HiveLiteNodeId::new(0x11223344)),
            1000,
        );
        let doc = CannedMessageDocument::new(event);
    ...
    fn test_document_merge() {
        let source = HiveLiteNodeId::new(0x111);
        let acker = HiveLiteNodeId::new(0x222);
    ...
    fn test_delta_op_encode_decode() {
        let source = HiveLiteNodeId::new(0x12345678);
        let acker1 = HiveLiteNodeId::new(0xAAAA);
        let acker2 = HiveLiteNodeId::new(0xBBBB);
    ...
    fn test_from_conversions() {
        ...
        assert_eq!(recovered.source_node, HiveLiteNodeId::new(0x999));
    }
}
