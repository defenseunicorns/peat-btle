...
impl ConnectionState {
    ...
    pub fn was_connected(&self) -> bool {
        matches!(
            self,
            Self::Connected | Self::Degraded | Self::Disconnecting | Self::Disconnected | Self::Lost
        )
    }
...
impl PeerConnectionState {
    ...
    pub fn from_peer(peer: &HivePeer, now_ms: u64) -> Self {
        ...
        Self {
            ...
            state,
            discovered_at: now_ms,
            connected_at: if peer.is_connected { Some(now_ms) } else { None },
            disconnected_at: None,
            disconnect_reason: None,
...
impl ConnectionStateGraph {
    ...
    pub fn get_with_history(&self) -> Vec<&PeerConnectionState> {
        self.peers.values().filter(|p| p.state.was_connected()).collect()
    }
