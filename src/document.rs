...
pub struct EcheDocument {
    ...
    /// Optional chat CRDT for mesh-wide messaging (requires `legacy-chat` feature)
    ///
...
impl EcheDocument {
    ...
    /// Create with an initial chat CRDT (requires `legacy-chat` feature)
    #[cfg(feature = "legacy-chat")]
    ...
    // --- Chat CRDT methods (requires `legacy-chat` feature) ---
    ...
    pub fn decode(data: &[u8]) -> Option<Self> {
        ...
        while offset < data.len() {
            ...
            if marker == EXTENDED_MARKER {
                ...
                // Skip/parse chat section (silently skip if legacy-chat not enabled)
                if data.len() < offset + 4 {
                    break;
                }
                let _reserved = data[offset + 1];
...
mod tests {
    ...
    // ============================================================================
    // Chat CRDT Document Tests (gated behind legacy-chat)
    // ============================================================================
